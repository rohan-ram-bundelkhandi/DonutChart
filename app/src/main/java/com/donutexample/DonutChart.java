...
public class DonutChart extends View {
    ...
    public DonutChart(Context context, AttributeSet attrs) {
        ...
        if(screenWidth>=720){
            ...
            try {
                radius = a.getDimension(R.styleable.DonutChart_radius, 110.0f);  //60 Normal
            } finally {
                a.recycle();
            }
        }
            ...
            try {
                radius = a.getDimension(R.styleable.DonutChart_radius, 60.0f);  //60 Normal
            } finally {
                a.recycle();
            }
        }

    //    Toast.makeText(context, "Width"+screenWidth+" Height"+screenHeight, Toast.LENGTH_LONG).show();
    ...
    //    drawDonut(canvas,paint, 0,359.9f);
        ...
        // green
        setGradient(0xff52D017,0xff52D017);
        drawDonut(canvas,paint, 0,MainActivity.correctAngle);

        //red
        setGradient(0xffe10000,0xffe10000);
        drawDonut(canvas,paint, MainActivity.correctAngle,MainActivity.incorrectAngle);

        // blue
        setGradient(0xff0000ff,0xff0000ff);
        if(MainActivity.incorrectAngle+MainActivity.correctAngle+MainActivity.unAttemptedAngle==360)
            drawDonut(canvas,paint, (MainActivity.incorrectAngle+MainActivity.correctAngle),MainActivity.unAttemptedAngle);
        else
            drawDonut(canvas,paint, (MainActivity.incorrectAngle+MainActivity.correctAngle),(360-(MainActivity.incorrectAngle+MainActivity.correctAngle)));
    }
